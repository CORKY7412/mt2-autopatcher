	...
	_In_ int       nCmdShow)
{
	...
	// Create window
	//hWnd = CreateWindowExW(0, Constants::GUID.c_str(), szWindowTitle, WS_POPUP | WS_MINIMIZEBOX, windowX, windowY, windowWidth, windowHeight, nullptr, nullptr, nullptr, nullptr);
	hWnd = CreateWindowExW(0, Constants::GUID.c_str(), szWindowTitle, WS_POPUP | WS_MINIMIZEBOX, windowX, windowY, windowWidth, windowHeight, nullptr, nullptr, nullptr, nullptr);

	if (!hWnd)
	{
		std::string err = "Failed to create window! (err: ";
...
void Main::constructLayout()
{
	...
	int optionsAreaHeight = 1 * menuButtonHeight;
...
void Main::initializeGraphics()
{
	...
	if (nNumFound == 0)
		throw std::logic_error("Niciun font gasit!");

	FontFamily fontFamilyBold;
	nNumFound = 0;
	boldFontCollection.GetFamilies(1, &fontFamilyBold, &nNumFound);
	if (nNumFound == 0)
		throw std::logic_error("Niciun bold font gasit!");
...
void Main::update()
{
	...
	if (optionsButton->isClicked())
	{
		...
		if (Helper::executeProgram(workingDirectory + L"\\" + Constants::GAMECONFIG_EXECUTABLE_NAME))
		{
		}
	}
	...
	if (patcher->statusText.empty())
	{
		...
		if (topProgressBar->value != patcher->fileDone || topProgressBar->maxValue != patcher->fileTotal)
		{
			...
			if (patcher->state == Patcher::STATE_UPDATING_LAUNCHER)
			{
				ss << L"Se descarca: " << Constants::GAME_LAUNCHER_FILE_NAME;
			...
			bottomProgressBar->value != patcher->totalBytesDone || bottomProgressBar->maxValue != patcher->totalBytesTotal)
		{
			...
			std::wostringstream ss;
			ss << L"Fisierul " << ((patcher->filesTotal == patcher->filesDone) ? patcher->filesDone : patcher->filesDone + 1) << L" din " << patcher->filesTotal;
			ss << L" (" << megabytesDownloaded << L" din " << megabytesToDownload << L" MB";
			ss << L", " << secondsRemaining / 60 << L" minute ramase)";
...
void Main::paint(HDC hdc)
{
	...
	if (options)
	{
		...
		std::wstring repairText = (repairButton->disabled && patcher->ignoreLocalVersionFile) ? L"Reparare..." : L"Repara";
