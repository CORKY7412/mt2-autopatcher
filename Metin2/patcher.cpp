...
Patcher::Patcher()
{
	...
	std::wstring oldVersionPath = Helper::getExecutableDirectory() + L"/old_metin2.bak";
...
void Patcher::run()
{
	...
	for (int i = 1; i <= Constants::NUMBER_OF_UPDATE_SERVERS; i++)
	{
		...
		std::string updateServer = Constants::UPDATE_DOMAIN;
...
bool Patcher::tryFromServer(std::string server, bool lastServer)
{
	...
	{
		smallStatusText = L"Actualizarea autopatcherului la v" + Helper::versionNumberToString(latestLauncherVersion);
		...
		if (updateLauncher(localDirectory, remoteDirectory, latestLauncherChecksum))
		{
			...
			state = STATE_SUCCESS;
			statusText = L"Autopatcherul a fost actualizat. Redeschide-l!";
			return true;
		}
	}
	...
	if (!isValidVersion(latestVersion))
	{
		...
		statusText = L"Nu s-a putut conecta la server!";
	...
	{
		state = STATE_SUCCESS;
		statusText = L"Cea mai recenta versiune este deja instalata!";
		return true;
	}
	...
	if (remoteChecksums.empty())
	{
		...
		statusText = L"Jocul nu a fost actualizat si este posibil sa nu functioneze corect:\nIncarcarea informatiilor nu a reusit!";
		return false;
	}
	...
	if (filesFailed)
	{
		...
		if (!continueWithFailedFiles(remoteFiles, L"Urmatoarele fisiere nu au putut fi verificate:\n"))
		{
			state = STATE_CHECKING_FILES_FAILED;
			statusText = L"Jocul nu a fost actualizat si este posibil sa nu functioneze corect:\nUnele fisiere locale nu au putut fi verificate!";
			return false;
		}
	}
	...
	if (filesToDownload == 0)
	{
		...
		{
			state = STATE_SUCCESS;
			statusText = L"Toate fisierele au fost verificate si sunt actualizate!";
			writeVersionFile(localDirectory, latestVersion);
		}
		else
		{
			state = STATE_CHECKING_FILES_FAILED;
			statusText = L"Jocul nu a fost actualizat si este posibil sa nu functioneze corect:\nNu exista fisiere de actualizat, dar unele nu au putut fi verificate!";
			return false;
		}
	}
	...
	if (localFaults)
	{
		...
		continueWithFailedFiles(remoteFiles, L"Urmatoarele fisiere nu au putut fi verificate:\n");
		state = STATE_DOWNLOADING_FILES_FAILED_WITH_LOCAL_ISSUES;
		statusText = L"Jocul nu a fost actualizat si este posibil sa nu functioneze corect:\nUnele fisiere au avut probleme. Este posibil sa fie in uz sau nu aveti acces la ele.";
		return false;
	}
	else if (failedFiles)
	{
		// no local issues, but some files still failed
		if (lastServer) continueWithFailedFiles(remoteFiles, L"Urmatoarele fisiere nu au putut fi verificate:\n");
		state = STATE_DOWNLOADING_FILES_FAILED;
		statusText = L"Jocul nu a fost actualizat si este posibil sa nu functioneze corect:\nUnele fisiere au avut probleme de descarcare.";
		return false;
	}

	// all files were successfully downloaded, mark the installation as up-to-date
	state = STATE_SUCCESS;
	statusText = L"Toate fisierele au fost actualizate cu succes!";
	writeVersionFile(localDirectory, latestVersion);
	return true;
}
...
int Patcher::getInstalledVersion(std::wstring installationDirectory)
{
	...
	statusText = L"Determinarea versiunii instalate...";
...
bool Patcher::getLatestVersion(std::string remoteDirectory, int &latestGameVersion, int &latestLauncherVersion, std::string &latestLauncherChecksum, int &numberOfUpdateServers)
{
	...
	statusText = L"Se verifica actualizarile...";
	...
	try
	{
		...
		currentFileName = L"Actualizare autopatcher";
...
bool Patcher::updateLauncher(std::wstring localDirectory, std::string remoteDirectory, std::string latestLauncherChecksum)
{
	...
	if (Downloader::downloadBinaryFile(remotePath, localPath, &fileDone, nullptr))
	{
		...
		if (sha256 == latestLauncherChecksum)
		{
			...
			std::wstring renamedCurrentExecutable = localDirectory + L"/old_metin2.bak";
			if (_wrename(currentExecutable.c_str(), renamedCurrentExecutable.c_str()) == 0)
			{
				// renamed metin2.exe -> old_metin2.bak

				if (_wrename(localPath.c_str(), currentExecutable.c_str()) == 0)
				{
					// renamed latest_launcher.exe -> metin2.exe
				...
				{
					// could not rename downloaded exe into metin2.exe
					// rename old_metin2.bak -> metin2.exe
...
std::string Patcher::getRemoteChecksumFile(std::string remotePatchDirectory)
{
	...
	statusText = L"Se descarca cele mai recente informatii...";
...
void Patcher::checkLocalFiles(std::wstring localDirectory, std::vector<PatchableFile> &remoteFiles, int &filesFailed, int &filesToDownload, int &bytesToDownload)
{
	...
	for (int i = 0; i < filesTotal; i++)
	{
		...
		if (f == NULL)
		{
			...
			currentFile->failReason = L"Deschiderea a esuat pentru verificare.";
			filesFailed++;
			continue;
		}
...
void Patcher::downloadFiles(std::wstring localDirectory, std::string remotePatchDirectory, std::vector<PatchableFile> &remoteFiles, int filesToDownload, int bytesToDownload, int &failedFiles, int &localFaults)
{
	...
	for (int i = 0; i < remoteFiles.size(); i++)
	{
		...
		if (Downloader::downloadBinaryFile(remoteUrl, compressedPath, &fileDone, &totalBytesDone))
		{
			...
			{
				currentFile->failed = true;
				currentFile->failReason = L"nu a putut fi deschis pentru verificare";
			...
			if (sha == currentFile->sha256)
			{
				...
				if (!Compression::decompressGzipFile(compressedPath, decompressedPath))
				{
					...
					currentFile->failed = true;
					currentFile->failReason = L"decomprimare nereusita";
					failedFiles++;
			...
			else
			{
				...
				currentFile->failed = true;
				currentFile->failReason = L"verificare nereusita";
				failedFiles++;
				continue;
			}
		...
		{
			currentFile->failed = true;
			currentFile->failReason = L"descarcare nereusita";
...
bool Patcher::continueWithFailedFiles(std::vector<PatchableFile> files, std::wstring message)
{
	...
	for (int i = 0; i < files.size(); i++)
	{
		...
		if (currentFile->failed)
		{
			if (filesDisplayed < 3)
			{
				prompt += L"\n" + Helper::s2ws(currentFile->name) + L" (Eroare: " + currentFile->failReason + L")";
				filesDisplayed++;
			}
	...
	{
		prompt += L"\n\n... si " + std::to_wstring(failedFiles - filesDisplayed) + L".";
	}
